    ...
    if existing_data and existing_data.get("worker") and existing_data.get("machine_no"):
        ...
        if u_submit_form_btn:
            ...
            if db_status_mob == "폐기" and u_status != "폐기":
                flow_error_msg = "⚠️ [공정 보안 경고] 이 툴은 이미 최종 '폐기' 처리가 완료된 상태입니다. 폐기 공구를 다시 가동 공정으로 되돌리는 것은 절대 불가능합니다!"
            elif db_status_mob == "재사용대기" and u_status in ["사용전", "사용중"]:
                flow_error_msg = "⚠️ [공정 보안 경고] 현재 보관('재사용대기') 중인 툴입니다. 재가동 시 무조건 [재사용] 또는 [폐기]만 선택해야 합니다!"
            elif db_status_mob == "사용전" and u_status in ["재사용", "재사용대기", "폐기"]:
                if u_status != "폐기":
                    flow_error_msg = f"⚠️ [공정 흐름 오류] 아직 가동된 적 없는 '사용전' 상태의 새 제품입니다. 바로 '{u_status}' 상태로 건너뛸 수 없습니다!"
            elif db_status_mob == "사용중" and u_status == "재사용":
                flow_error_msg = "⚠️ [공정 흐름 오류] 현재 '사용중'인 툴은 바로 '재사용'으로 갈 수 없습니다! 반드시 먼저 '재사용대기'를 선택해야 합니다."
            elif db_status_mob in ["사용중", "재사용", "재사용대기"] and u_status == "사용전":
                flow_error_msg = "⚠️ [공정 오류] 이미 사용 흔적이 기록된 가동 툴은 '사용전' 복구가 불가합니다! PC 대시보드 하단의 '완전 초기화' 기능을 이용하세요."
            elif u_status in ["사용중", "재사용", "재사용대기"] and (not u_worker or u_machine_num == 0):
                flow_error_msg = "⚠️ [데이터 누락] 가동/보관 단계 저장 시에는 [교체 작업자 이름] 및 [기계 가공 호기]를 반드시 입력해야 합니다!"
        ...
        if not is_valid and db_status_mob == "사용전" and u_status == "폐기":
            is_valid = True
        ...
        with st.form(key="mobile_input_form"):
            ...
            # ★ [현장 모바일 최초 기입] 마스터 스펙 동적 연동 및 종류 판별 규칙 적용
            st.markdown("🛠 **상세 스펙 선택 (마스터 리스트)**")
            spec_master_col = get_spec_master_collection()
            tool_code_pref = qr_scanned_serial[:1] # ◀ [검증 완료] 12자리 규칙에 맞춰 맨 앞 1자리로 종류 판별 분기 교정
