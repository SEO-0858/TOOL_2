...
def show_reuse_pending_dialog(s_no, current_mach, orig_note, ed_worker, ed_machine_num, ed_hours, ed_mins, selected_spec=None):
    ...
    if st.button("🚀 실적 기록 및 재사용대기 저장"):
        ...
        if selected_spec:
            update_fields["detail_spec"] = selected_spec
...
def show_waste_dialog(s_no, current_mach, orig_note, ed_worker, from_status, selected_spec=None):
    ...
    if chosen_reason == "5. 기타 (직접기입)":
        detail_reason = st.text_input("📝 상세 폐기 사유 직접 입력", placeholder="예: 낙하 파손", key=f"pop_detail_reason_{s_no}").strip()
...
if qr_scanned_serial:
    ...
    if existing_data and existing_data.get("worker") and existing_data.get("machine_no"):
        ...
        with st.form(key="mobile_update_form"):
            ...
            # ★ [현장 모바일] 관리자가 등록한 마스터 스펙 동적 연결
            st.markdown("🛠 **상세 스펙 선택 (마스터 리스트 호출)**")
            client = MongoClient(st.secrets["database"]["MONGO_URI"])
            spec_master_col = client["dashboard_db"]["tool_specs_master"]
            ...
            u_spec = st.selectbox("관리자가 작성한 리스트에서 스펙을 골라주세요", spec_options, index=def_idx, key="mob_spec_select_box")
            ...
            if "현장 입고일" in default_val or "QR 선발행" in default_val:
                ...
                display_note = match.group(1) if match else ""
        ...
        if u_submit_form_btn:
            ...
            if db_status_mob == "폐기" and u_status != "폐기":
                flow_error_msg = "⚠️ 이 툴은 이미 최종 '폐기' 처리가 완료된 상태입니다!"
            elif db_status_mob == "재사용대기" and u_status in ["사용전", "사용중"]:
                flow_error_msg = "⚠️ 다시 장착하여 재가동할 때는 '재사용' 또는 '폐기'만 선택해야 합니다!"
            elif db_status_mob == "사용전" and u_status in ["재사용", "재사용대기", "폐기"]:
                if u_status != "폐기":
                    flow_error_msg = f"⚠️ 아직 가동된 적 없는 새 제품입니다. 바로 '{u_status}' 상태로 갈 수 없습니다!"
            elif db_status_mob == "사용중" and u_status == "재사용":
                flow_error_msg = "⚠️ 사용중인 툴은 바로 재사용으로 갈 수 없습니다. 재사용대기를 먼저 거쳐야 합니다."
            elif u_status in ["사용중", "재사용", "재사용대기"] and (not u_worker or u_machine_num == 0):
                flow_error_msg = "⚠️ 가동/보관 단계 저장 시에는 [작업자 이름] 및 [기계 가공 호기]를 필수 입력해야 합니다!"
            elif u_status == "폐기" and not u_worker:
                flow_error_msg = "⚠️ 툴 폐기 처리를 하려면 [작업자 이름]을 반드시 입력해야 합니다!"
        ...
        with st.form(key="mobile_input_form"):
            ...
            # ★ [최초 빈 QR 등록 시] 마스터 리스트 연동하여 스펙 지정하게 하기
            st.markdown("🛠 **상세 스펙 선택 (마스터 리스트 호출)**")
            client = MongoClient(st.secrets["database"]["MONGO_URI"])
            spec_master_col = client["dashboard_db"]["tool_specs_master"]
            ...
            mapped_t_type = "전착툴" if tool_code_pref=="001" else "레진툴" if tool_code_pref=="002" else "메탈툴" if tool_code_pref=="003" else "코어툴"
            
            db_specs = list(spec_master_col.find({"main_type": mapped_t_type}))
            if db_specs: spec_options = [spec["spec_name"] for spec in db_specs]
            else: spec_options = ["기본 스펙"]
                
            m_spec = st.selectbox("등록할 스펙을 골라주세요", spec_options, key="mob_init_spec_select")
        ...
        if submit_m_btn:
            if not m_worker: add_error("⚠️ 작업자 이름을 반드시 입력해 주세요!")
                ...
                db_collection.update_one(
                    ...
                    {"$set": {
                        "serial_no": qr_scanned_serial,
                        "tool_type": mapped_t_type,
                        "status": m_status,
                        "input_date": str(chosen_date), 
                        "init_time": chosen_time.strftime("%H:%M"),  
                        "worker": m_worker,
...
# --- 💻 [PC 관리자 모드] ---
    ...
    if tool_menu == "📊 빈데이터 QR코드 대량 선발행":
        ...
        c1, c2 = st.columns(2)
        with c1: tool_code = st.text_input("🆔 고유넘버 앞 3자리 입력 (전착:001 / 레진:002 / 메탈:003 / 코어:004)", value="001", max_chars=3)
        with c2: quantity = st.number_input("📦 발행할 QR코드 갯수", min_value=1, max_value=100, value=50, step=1)
            
        prefix = f"{tool_code}{mmdd}"
        try:
            last_tool = db_collection.find_one({"serial_no": {"$regex": f"^{prefix}"}}, sort=[("serial_no", -1)])
            last_counter = int(last_tool["serial_no"][-5:]) if last_tool else 0
        except Exception: last_counter = 0
        ...
        if st.button(f"🖨️ {quantity}개의 연속 QR코드 즉시 발행"):
            ...
            mapped_type_str = "전착툴" if tool_code=="001" else "레진툴" if tool_code=="002" else "메탈툴" if tool_code=="003" else "코어툴"
            ...
            for idx in range(1, quantity + 1):
                ...
                blank_records.append({
                    ...
                    "current_use": 0,
                    "waste_date": "-",
                    "detail_spec": "스펙미지정", # 발행 시점엔 빈 상태(스펙미지정)로 고정하여 발행 타임 단축
            ...
            with st.expander("💥 데이터베이스 초기화 및 특정 시리얼 개별 삭제", expanded=False):
                ...
                if delete_mode == "📂 종류별 묶음 초기화 및 리셋":
                    ...
                    if st.button("❌ 해당 개별 시리얼 넘버 데이터 즉시 삭제", key="btn_single_del"):
                        if len(target_single_serial) != 12: st.error("⚠️ 시리얼 번호는 정확히 12자리여야 합니다.")
                        elif not understand_risk_single: st.error("⚠️ 영구 삭제 동의 체크박스를 체크해 주세요.")
                        else:
                            if db_collection.count_documents({"serial_no": target_single_serial}) == 0: st.error("❌ 데이터가 존재하지 않습니다.")
                            else:
                                db_collection.delete_one({"serial_no": target_single_serial})
                                st.session_state.reset_message = f"🎯 지정 시리얼 [`{target_single_serial}`] 영구 삭제 완료!"; st.session_state.reset_success = True; st.rerun()
            ...
            if not all_data: st.info("조회할 데이터가 없습니다.")
                ...
                if not filtered_data: st.warning("🔍 일치하는 툴 데이터가 없습니다.")
                    ...
                    for item in filtered_data:
                        ...
                        status_badge = "🟢 [사용전]" if db_current_status == "사용전" else "🟡 [사용중]" if db_current_status == "사용중" else "🔵 [재사용]" if db_current_status == "재사용" else "🟣 [재사용대기]" if db_current_status == "재사용대기" else "🔴 [폐기]"
                        ...
                        with st.expander(expander_title):
                            ...
                            if st.session_state[edit_key]:
                                ...
                                with st.form(key=f"board_edit_form_{s_no}"):
                                    ...
                                    ed_spec = st.selectbox("관리자가 작성한 리스트에서 스펙을 선택하세요", spec_options, index=def_idx, key=f"spec_pc_select_{s_no}")
                                    ...
                                    ed_limit = st.number_input("⚙️ Limit 사용 한도 재설정", min_value=0, value=int(item.get('use_limit', 10000)), step=1000, key=f"lim_{s_no}")
                                ...
                                if b_submit:
                                    flow_error_msg = ""
                                    if db_current_status == "폐기" and ed_status != "폐기": flow_error_msg = "⚠️ 이미 폐기가 끝난 툴은 되돌릴 수 없습니다!"
                                    if flow_error_msg: add_error(flow_error_msg); st.stop()
                                    
                                    is_valid, msg = validate_process(db_current_status, ed_status)
                                    if not is_valid and db_current_status == "사용전" and ed_status == "폐기": is_valid = True
                                    ...
                                    final_note_val = ed_note.strip() + f"\n[{get_now_kst().strftime('%Y-%m-%d %H:%M:%S')}] 변경완료"
                                    db_collection.update_one(
                                        {"serial_no": s_no},
                                        {"$set": {
                                            "status": ed_status,
                                ...
                                if st.checkbox(f"❗ 가동 내역 완전 강제 초기화 승인", key=f"risk_reset_{s_no}"):
                                    if st.button("🗑️ 초기화 실행", key=f"btn_reset_{s_no}", type="primary"):
                                        db_collection.update_one({"serial_no": s_no}, {"$set": {"status": "사용전", "worker": "", "machine_no": "", "dressing_hours": 0, "dressing_mins": 0, "start_time": "-", "target_time": "-", "waste_date": "-", "current_use": 0, "note": "수동 초기화 완료", "history": []}})
                                        st.success("리셋 완료"); time.sleep(0.5); st.rerun()
                                ...
                                if st.button("✏️ 정보 직접 수정", key=f"btn_edit_{s_no}"): st.session_state[edit_key] = True; st.rerun()
    ...
    # 4) 데이터 수정 / 삭제 / QR 재발행 창
        ...
        if target_serial and len(target_serial) == 12:
            ...
            if exist_item:
                ...
                st.image(qr_res_bytes, width=180, caption=f"재발행: {target_serial}")
                ...
                if st.button(f"➕ 누락번호 `{target_serial}` 신규 생성 및 QR 발행"):
                    ...
                    db_collection.insert_one({"serial_no": target_serial, "tool_type": "전착툴" if t_code=="001" else "레진툴" if t_code=="002" else "메탈툴", "status": "사용전", "input_date": str(today), "init_time": get_now_kst().strftime("%H:%M"), "worker": "", "machine_no": "", "dressing_hours": 0, "dressing_mins": 0, "start_time": "-", "target_time": "-", "use_limit": 10000, "current_use": 0, "waste_date": "-", "detail_spec": "스펙미지정", "note": "강제 재발행 완료"})
                    st.success("신규 생성 완료"); st.rerun()

    # 5) 🖥️ 실시간 기계 정보창 (Grid Layout)
    elif tool_menu == "🖥️ 실시간 기계 정보창":
            st.title("🖥️ 실시간 기계 배치 및 툴 상세 현황")
            st.write(f"⏰ **현재 기준 시간:** {get_now_kst().strftime('%Y-%m-%d %H:%M:%S')}")
            layout = [
                        ...
                        with st.container(border=True, height=160):
                            ...
                            if tools:
                                for t in tools:
                                    ...
                                    st.markdown(f"<div style='font-size:9px; color:black; line-height:1.2; margin-top:2px;'><b>ID:</b> {t.get('serial_no')}<br><b>스펙:</b> {t.get('detail_spec','-')}<br><b>작업자:</b> {t.get('worker')}<br><span style='color:red;'>{elapsed}</span></div>", unsafe_allow_html=True)
                            else: st.markdown("<div style='color:black; font-size:11px;'>대기중</div>", unsafe_allow_html=True)

    # -------------------------------------------------------------
    # 6) 🔧 툴 상세스펙 마스터 관리 (새롭게 완전히 매립된 하위 메뉴)
    # -------------------------------------------------------------
    elif tool_menu == "🔧 툴 상세스펙 마스터 관리":
        st.title("🔧 툴 상세 스펙 마스터 관리")
        st.write("여기서 스펙을 등록해 두면, 현장 작업자가 QR 코드를 스캔했을 때 선택 목록으로 자동으로 불려갑니다.")
        
        client = MongoClient(st.secrets["database"]["MONGO_URI"])
        spec_master_col = client["dashboard_db"]["tool_specs_master"]
        
        with st.form("spec_input_form_master", clear_on_submit=True):
            st.subheader("➕ 새로운 툴 상세 스펙 리스트에 등록")
            ins_type = st.selectbox("1. 툴 대분류 선택", ["전착툴", "레진툴", "메탈툴", "코어툴"])
            ins_name = st.text_input("2. 상세 스펙 이름 입력", placeholder="예: 파이90-20-200메쉬").strip()
            ins_memo = st.text_input("3. 기타 비고 및 메모", placeholder="예: A사 수입품 / 입도 #400")
            
            if st.form_submit_button("💾 스펙 리스트에 등록"):
                if not ins_name: st.error("⚠️ 스펙 이름을 정확히 입력해 주세요.")
                else:
                    spec_master_col.insert_one({"main_type": ins_type, "spec_name": ins_name, "memo": ins_memo})
                    st.success(f"🎉 '{ins_name}' 스펙이 마스터 목록에 안전하게 보관되었습니다!")
                    time.sleep(0.5); st.rerun()

        st.write("<br><hr>", unsafe_allow_html=True)
        st.subheader("📋 현재 등록되어 사용 중인 공용 스펙 목록")
        all_specs_list = list(spec_master_col.find({}))
        
        if not all_specs_list: st.info("💡 아직 등록된 스펙 목록이 없습니다. 위 폼에서 리스트를 개설해 주세요.")
        else:
