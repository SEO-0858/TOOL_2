...
if qr_scanned_serial:
    ...
    # @st.fragment를 사용하여 이 영역만 따로 갱신 (화면 전체 깜빡임 방지)
    @st.fragment
    def mobile_form_fragment(qr_serial):
        # 1. 데이터 조회
        existing_data = db_collection.find_one({"serial_no": qr_serial}) or {}
        ...
        u_note = st.text_area("📝 현장 특이사항", value=saved_note, height=150)
    ...
    # 프래그먼트 호출
    mobile_form_fragment(qr_scanned_serial)
