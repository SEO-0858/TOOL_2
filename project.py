...
# --- 💻 [PC 관리자 모드] ---
    ...
    # 5) 🖥️ 실시간 기계 정보창 (새로고침 버튼 포함된 원본 유지 버전)
    elif tool_menu == "🖥️ 실시간 기계 정보창":
        st.title("🖥️ 실시간 기계 배치 및 툴 상세 현황")
        
        # [추가] 데이터 갱신 버튼: 기존 코드에 없던 기능으로, 누락 없이 최신 상태를 불러옵니다.
        if st.button("🔄 실시간 데이터 강제 갱신"):
            st.rerun()
                        ...
                        if tools:
                            for t in tools:
                                ...
                                <div style='font-size:13px; color:black; line-height:1.5; margin-top:5px; font-weight:bold;'>
                                    ...
                                    <span style='color:red; font-size:18px;'>⏳ {elapsed}</span>
                                </div>
                                """, unsafe_allow_html=True)
                        else:
