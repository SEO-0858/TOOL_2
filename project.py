            ...
            if not all_data:
                ...
                if not filtered_data:
                    ...
                    for item in filtered_data:
                        ...
                        # 폼 제출 버튼 확인 로직이 이 안으로 들어와야 합니다!
                        if st.form_submit_button("💾 저장"): 
                            update_time = get_now_kst().strftime("%m/%d %H:%M")
                            new_note_entry = f"\n[{update_time} 수정] 상태:{ed_status}, 작업자:{ed_worker}, 기계:{ed_machine}호기"
                            ...
                            db_collection.update_one(
                                {"serial_no": s_no},
                                {
                                    "$set": {
                                        "status": ed_status,
                                        "worker": ed_worker,
                                        "machine_no": f"{ed_machine}호기",
                                        "note": item.get('note', '') + new_note_entry
                                    }
                                }
                            )
                            st.session_state[edit_key] = False
                        ...
                        with col_y:
                            ...
                            st.write(f"• **⏳ 드레싱 주기:** {item.get('dressing_hours', 0)}시간")
