...
if qr_scanned_serial:
    ...
    # 2. 상태 전이 규칙 (사용자 요구사항 반영: 가동전->재사용 차단 등)
    status_map = {
        "가동전": ["사용중", "폐기"],
        "사용중": ["재사용대기", "폐기"],
        "재사용대기": ["재사용", "폐기"],
    ...
    current_status = db_status_mob if db_status_mob in status_map else "가동전"
    allowed_options = status_map.get(current_status, [])
    ...
    with st.form(key="mobile_update_form"):
        ...
        # 상태 선택 (allowed_options를 통해 원천 차단)
        if allowed_options:
            u_status = st.radio("🔄 툴 현재 상태 선택", allowed_options, index=0, horizontal=True)
        else:
            st.warning("⚠️ 이 툴은 더 이상 상태 변경이 불가능합니다.")
            u_status = current_status
        
        # [조건부 필수 입력] '사용중'에서 '재사용대기'나 '폐기'로 갈 때만 수량 입력 활성화
    ...
    # 4. 저장 로직 (필수 입력 검증 및 DB 업데이트)
    if u_submit_form_btn:
        if show_count_input and u_work_count == 0:
            ...
            new_log = ""
            if is_status_changed or is_spec_changed or show_count_input:
                new_log = f"\n[{log_time_str}] 상태:{db_status_mob}→{u_status}"
                if is_spec_changed: new_log += f", 스펙:{old_spec}→{u_spec}"
